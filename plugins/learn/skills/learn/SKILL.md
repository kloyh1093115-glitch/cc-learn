...
```
1. トピックが属する分野を特定する
2. そのトピックは基盤 + knowledge/ の既存の章でカバーされているか？
...
```
対応:
  1. コンテキストの references/ から依存関係と該当セクションの行番号を特定する
...
基盤は線形代数・解析学・統計幾何学の3冊。各セクションの本質・数式・依存関係は起動時に読み込む references/ にある。
教科書本体は textbooks/ にある。詳しい解説が必要なときはコンテキストの references/ の行番号で textbooks/ の該当箇所を Read する。
